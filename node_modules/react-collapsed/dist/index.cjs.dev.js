...
function useCollapse(_a = {}) {
  ...
  return {
    ...
    setExpanded,
    getToggleProps(rest) {
      const { disabled, onClick, refKey } = __spreadValues({
        refKey: "ref",
        onClick() {
        },
        disabled: false
      ...
      if (isButton === false) {
        return __spreadValues(__spreadValues({}, props), fakeButtonProps);
      } else if (isButton === true) {
        return __spreadValues(__spreadValues({}, props), buttonProps);
      } else {
        return __spreadValues(__spreadValues(__spreadValues({}, props), buttonProps), fakeButtonProps);
      }
    },
    getCollapseProps(rest) {
      const { style, refKey } = __spreadValues({ refKey: "ref", style: {} }, rest);
      const theirRef = rest == null ? void 0 : rest[refKey || "ref"];
